...
export default function EditTCPage() {
  ...
  const { data: tc, isLoading: tcLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<TechnicalCondition>(
        `/technical-conditions/${id}`
      );
      return res.data;
    },
    enabled: !!id && isAdmin,
    ...
    onError: (error: ApiError) => {
      setServerError(
        error.response?.data?.error ||
          'Ошибка при обновлении ТУ. Попробуйте снова.'
      );
    },
  });
  ...
  if (!tc) {
    return (
      <div className={styles.container}>
        <h2>ТУ не найдено</h2>
        <Link href="/admin/tc-management">← к списку ТУ</Link>
      </div>
    );
  }
